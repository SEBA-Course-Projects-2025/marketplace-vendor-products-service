...
// @Description  Returns a single product by its slug for the given vendor.
// @Tags         products
// @Accept       json
// @Produce      json
...
func (h *ProductHandler) GetProductBySlugHandler(c *gin.Context) {

	ctx, span := tracer.Tracer.Start(c.Request.Context(), "GetProductBySlugHandler")
	defer span.End()
	...
	product, err := services.GetProductBySlug(ctx, h.ProductRepo, slug, vendorId)
	if err != nil {
