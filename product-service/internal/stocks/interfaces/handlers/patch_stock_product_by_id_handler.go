...
func (h *StockHandler) PatchStockProductByIdHandler(c *gin.Context) {
	...
	ctx, span := tracer.Tracer.Start(c.Request.Context(), "PatchStockProductByIdHandler")
	defer span.End()
	...
	stock, err := services.PatchStockProductById(ctx, h.StockRepo, h.ProductRepo, h.EventRepo, h.Db, stockProductReq, stockId, productId, vendorId)
